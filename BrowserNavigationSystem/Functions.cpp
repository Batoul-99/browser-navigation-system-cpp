...
string deleteSpaces(string text) {
	...
	size_t startpos = text.find_first_not_of(' '); //here find first postition that isn't a space
	if (startpos == string::npos) return ""; //here if the whole string iss spaces return an empty string 
	text.erase(0, startpos); //here delete till startpos-1

	//Now I want to delete spaces from end
	size_t endpos = text.find_last_not_of(' '); //here also find last pos not space 
	if (endpos == string::npos) return "";
	text.erase(endpos + 1); //erase from endpos+1 till end
